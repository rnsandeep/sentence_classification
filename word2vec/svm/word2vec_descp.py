...
def preprocessSentences(file_name):
    ...
    df = df[~df['title'].isna()]
    df['description_lower'] = df['description'].apply(lambda x:lower_case(x))
    stop_words = set(stopwords.words('english'))
    df['description_punc'] = df['description_lower'].apply(lambda x:remove_punctuation(x))
    df['stop_removal'] = df['description_punc'].apply(lambda x:remove_stop_words(x))
...
def getWord2vecTrainFeatures(file_name):
    ...
    pickle.dump(model_word2vec.wv, open('word2vec_wv.pkl','wb'))
...
test_features, test_labels = getWord2VecFeatures('validation.csv', word2vec_model)

clf = LinearSVC(random_state=0, tol=1e-5).fit(train_features, train_labels)
